...
class myNode():
    ...
    def __init__(self, nodeid, position, transmitParams, initial, sfSet, freqSet, powSet, bsList,
        ...
        if algo == 'exp3':
            ...
            self.alpha = None
            self.exploration_constant = np.sqrt(2 * np.log(horTime))
        
        # weight and prob for learning
        if algo == 'ucb':
            self.action_count = {x: 0 for x in range(0, self.nrActions)}
            self.action_reward_sum = {x: 0.0 for x in range(0, self.nrActions)}
            self.action_mean = {x: 0.0 for x in range(0, self.nrActions)}
    ...
    def updateProb(self, algo):
        """ Update the probability of each action by using EXP3 or UCB algorithm.
        Parameters
        ----------
        ...
        """
        if algo == 'ucb':
            self._updateProb_UCB()
        else:
            self._updateProb_EXP3(algo)
    
    def _updateProb_UCB(self):
        """ Update using UCB (Upper Confidence Bound) algorithm.
        """
        # Compute reward for the chosen action
        reward = 0
        ...
        # Update statistics for the chosen action
        action_idx = self.packets[0].choosenAction
        self.action_count[action_idx] += 1
        self.action_reward_sum[action_idx] += reward
        self.action_mean[action_idx] = self.action_reward_sum[action_idx] / self.action_count[action_idx]
        
        # Compute total number of pulls
        total_pulls = sum(self.action_count.values())
        
        # Compute UCB indices and probabilities
        ucb_indices = np.zeros(self.nrActions)
        for j in range(self.nrActions):
            if self.action_count[j] == 0:
                ucb_indices[j] = float('inf')  # Unexplored actions have infinite UCB
            else:
                exploitation = self.action_mean[j]
                exploration = self.exploration_constant * np.sqrt(np.log(total_pulls) / self.action_count[j])
                ucb_indices[j] = exploitation + exploration
        
        # Convert UCB indices to probabilities (greedy: always select max UCB)
        best_action = np.argmax(ucb_indices)
        prob = np.zeros(self.nrActions)
        prob[best_action] = 1.0
        
        # Add small exploration for diversity
        epsilon = 0.01
        prob = (1 - epsilon) * prob + (epsilon / self.nrActions)
        
        self.prob = {x: prob[x] for x in range(0, self.nrActions)}
    
    def _updateProb_EXP3(self, algo):
        """ Update using EXP3 or EXP3s algorithm.
        """
